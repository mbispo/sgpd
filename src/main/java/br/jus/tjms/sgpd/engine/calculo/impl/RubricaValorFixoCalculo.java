...
public class RubricaValorFixoCalculo implements RubricaCalculo {
	
	private static final long serialVersionUID = 5292925414226284045L;
	
	static RubricaCalculo instance;
	
	public static RubricaCalculo newInstance() {
		return new RubricaValorFixoCalculo();
	}
	
	public static RubricaCalculo instance() {
		if (instance == null) {
			instance = new RubricaValorFixoCalculo(); 
		}
	...
	/*
	 * :Rubrica:
	 * rubrica possui parâmetros?
	 * Sim: retorna o valor fixo definido no parâmetro ativo (ver rubrica.rubricaParametros.vigenciaInicio e rubrica.rubricaParametros.vigenciaFim)
	 * Não: retorna o valor definido na rubrica (ver rubrica.valor)
	 * 
	 * :Rubrica do funcionário:
	 * rubrica do funcionário possui parâmetro valor definido?
	 * Sim: retorna o valor definido (ver rubricaFuncionario.valor)
	 * Não: pega a rubricaFuncionario.rubrica e vai para o cálculo :Rubrica
	 * 
	 */
	...
	public RubricaCalculoResultado calcular(Contexto contexto, Rubrica rubrica) {
		...
		List<RubricaParametro> parametrosAtivos = rubrica.getParametrosAtivos(new Date());
